const _ = () => {
  return (
    <div className="flex flex-col items-center justify-center gap-2 w-full p-8 pt-4">
        <div className="flex gap-2">
          <h5 className="text-[#3f64ef] underline">Twitter</h5>
          <h5 className="opacity-50">•</h5>
          <h5 className="text-[#3f64ef] underline">Github</h5>
          <h5 className="opacity-50">•</h5>
          <h5 className="text-[#3f64ef] underline">Security</h5>
          <h5 className="opacity-50">•</h5>
          <h5 className="text-[#3f64ef] underline">Blog</h5>
          <h5 className="opacity-50">•</h5>
          <h5 className="text-[#3f64ef] underline">Legal</h5>
        </div>
        <div className="flex gap-2">
          <h5 className="opacity-50">©2010</h5>
          <h5 className="text-[#3f64ef] underline">dev/finance inc</h5>
        </div>
      </div>
  )
}

export default _;